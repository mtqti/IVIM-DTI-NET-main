...
def learn_IVIM(X_train, bvalues, arg, net=None):
   ...
   # np.delete(X_train, isnan(np.mean(X_train, axis=1)), axis=0)
    X_train = np.delete(X_train,   np.isnan(np.mean(X_train, axis=1)), axis=0)
...
def predict_IVIM(data, bvalues, net, arg):
    ...
    # ---- CLAMP S0 (CRITICAL FIX) ----
    S0[~np.isfinite(S0)] = 1.0   # handles NaN / Inf
    S0[S0 == 0] = 1.0            # prevents divide-by-zero
    data = data / S0[:, None]
   #np.delete(data, isnan(np.mean(data, axis=1)), axis=0)
    data = np.delete(data, np.isnan(np.mean(data, axis=1)), axis=0)
    ...
    sels = sels & (np.percentile(data[:, bvalues < 50], 95, axis=1) < 1.3) & (
                np.percentile(data[:, bvalues > 50], 95, axis=1) < 1.2) & (
                       np.percentile(data[:, bvalues > 150], 95, axis=1) < 1.0)
...
def checkarg_net_pars(arg):
    ...
    if not hasattr(arg,'cons_max'):
        ...
        arg.cons_max = [0.0015, 0.40, 0.005, 0.05,  0.2, 1.1]  # Dpar, Fint, Dint, Fmv, Dmv, S0
    ...
    if not hasattr(arg,'IR'):
        ...
        arg.IR = True   #accounting for inversion recovery, True=yes, False=no
...
def checkarg_sim(arg):
    if not hasattr(arg, 'bvalues'):
        ...
        arg.bvalues = [0, 30, 90, 210, 280, 350, 580, 620, 660, 680, 720, 760, 980, 990, 1000]
    ...
    if not hasattr(arg, 'range'):
        ...
        arg.range =np.array (  [[0.0001, 0.0, 0.0015, 0.0, 0.004], [0.0015, 0.40, 0.004, 0.2, 0.2]]) # Dpar, Fint, Dint, Fmv, Dmv
    return arg
...
class net_pars:
    def __init__(self):
        ...
        self.cons_max = [0.0015, 0.40, 0.004, 0.05, 0.2, 1.1]  # Dpar, Fint, Dint, Fmv, Dmv, S0
        ####
...
class sim:
    def __init__(self):
        ...
        self.range = ([0.0001, 0.0, 0.0015, 0.0, 0.004], [0.0015, 0.7, 0.004, 0.2, 0.2]) # Dpar, Fint, Dint, Fmv, Dmv
...
class rel_times:
    def __init__(self):
       ...
       #self.isfT1 =  1250 # ms [Wong et al. JMRI (2020)]
        ...
        self.echotime = 58 # ms
        self.repetitiontime = 18000 # ms
        self.inversiontime = 2330 # ms
